...
//			if (DEBUG) Log.i(TAG, "unregister:");
	...
	public List<UsbDevice> getDeviceList(final List<DeviceFilter> filters) throws IllegalStateException {
		...
		//String fileName = Environment.getRootDirectory()+ "/USBCamera/failed_devices.txt";

		File logFile=null;
		/*try {
			String rootPath = Environment.getExternalStorageDirectory()
					.getAbsolutePath() + "/USBCamera/";
			File root = new File(rootPath);
			if (!root.exists()) {
				root.mkdirs();
			}
			logFile = new File(rootPath + "failed_devices.txt");
			if (!logFile.exists()) {
		...
		*//*	FileOutputStream out = new FileOutputStream(logFile);
		...
		/*	fw = new FileWriter(logFile, true);
			...
			if (deviceList != null) {
				...
				} else result.addAll(deviceList.values());
